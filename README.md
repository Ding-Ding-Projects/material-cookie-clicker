...
The generator ladder now has twenty-one real tiers. The **Office Buildings**
expansion adds the ninth tier, **Office Building · 寫字樓**, positioned between
Wizard Tower and Shipment, and it is
permanently revealed when the all-time baked-cookie counter reaches exactly
500,000,000. The reveal is based on lifetime production rather than the current
cookie balance: spending cookies cannot hide the row again, and prestige cannot
remove it from the save. The existing 100,000 Minigames unlock, 1,000,000 Mouse
...
| --- | --- |
| Superseded fresh-state baseline: the current opening removes the pictured title bar, HUD, console and surrounding panel until graphics are explicitly purchased | ![The superseded baseline fresh state: a white page with a plain COOKIE disc, plus title-bar purchase plates, a cookie counter, Prices and Settings controls that are no longer visible in the current cookie-only opening.](captures/app/plain-start.png) |
